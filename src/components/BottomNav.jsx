...
export default function BottomNav({ tab, setTab, cartCount = 0 }) {
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 flex justify-center z-40">
      <div className="mx-auto max-w-md w-full px-2 py-2 shadow-sm flex-1 bg-white border border-brand-primary rounded-[3px]">
        <div className="flex items-center justify-between">
          {items.map((it) => {
            const isActive = tab === it.key;
            const IconComponent = it.icon;
            const iconColor = isActive ? 'text-brand-primary' : 'text-zinc-400';
